...
function displayUI(theme, html, rawText) {
	...
	content += html;
	content += "<div id=\"rawText\" style=\"display:none;\">"+rawText+"</div>";
	...
	viewSourceElement.href = "javascript:void(0);";
	viewSourceElement.addEventListener('click', viewSource, false);
...
function viewSource(event){
	let target = event.target;
	let element = document.getElementById("rawText");
	let json = document.getElementById("json");
    if(element.style.display == "none"){
		element.style.display = "block";
		json.style.display = "none";
		target.text = "View json";
    } else{
		element.style.display = "none";
		json.style.display = "block";
		target.text = "View source";
    }
}
...
function init(data) {
	port.onMessage.addListener(function(msg) {
		...
		if (msg.onjsonToHTML)
			if (msg.html) {
				displayUI(msg.theme, msg.html, data.text);
			} else if (msg.json)
				port.postMessage({
					getError : true,
					json : json,
